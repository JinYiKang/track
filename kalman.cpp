...
void Kalman::set_external_noise(const Eigen::MatrixXd &noise_cov)
{
...
void Kalman::predict()
{
  stat_pred_ = predict_matrix_ * stat_refine_;
  stat_cov_pred_ = predict_matrix_ * stat_cov_refine_ * predict_matrix_.transpose() + external_noise_cov_;
}
...
void Kalman::correct(const Eigen::VectorXd &measure_stat, const Eigen::MatrixXd &measure_noise_cov)
{
  calc_kalman_gain_(measure_noise_cov);
  stat_refine_ = stat_pred_ + gain_matrix_ * (measure_stat - measure_trans_ * stat_pred_);
